...
| Laminar coolant film | Shah-London all-walls-uniform-heat-flux rectangular-duct polynomial in `alpha = min(w,h)/max(w,h)` | `Fluid Mechanics, 7th Ed. (Mcgraw-Hill Series in Mechanical Engineering).pdf`, noncircular-duct section and its Shah & London source route | Software-verified fully developed laminar branch for actual rectangular regen calls. Unequal wall heating and hydrodynamic/thermal entrance effects remain unresolved. `Nu = 4.36` is retained only by the lower-level backward-compatible circular-duct API when rectangular geometry is omitted. |
