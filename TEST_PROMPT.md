...
13 kN LOX/RP-1 engine, sea-level matched expansion (Pc 3.0 MPa, epsilon 4.524, Pe = Pa = 101325 Pa), non-scalar regen wall (station-wise GRCop-84 hot-wall + channel thermostructural sizing, Inconel 718 jacket), pintle injector, electric pumps, all component CAD exported as STL. All parameters live in `examples/cli/test_13kn_sealevel_regen_allstl.args` — do not change them; they are literature-anchored (provenance in the file's header comments).

## Step 0 — preflight

Check `python -c "import cadquery"`. If missing, run `pip install cadquery` (needed for pintle part STLs). If it cannot be installed, continue but mark the pintle STL check SKIPPED-NO-CADQUERY in the report.

## Step 1 — fast pipeline check (default, run every time)

```bash
PYTHONPATH=. python -m raosim @examples/cli/test_13kn_sealevel_regen_allstl.args
```

Runs the seed contour (`--max-nfev 0`) through the entire pipeline: thrust-sized throat, chamber, station-wise regen sizing, thermal, injector, pumps, all STL exports. Takes roughly a minute. Capture stdout/stderr to a log. A nonzero exit code is an automatic FAIL: report the failing gate or traceback and stop.

## Step 2 — artifact completeness (all CAD present, all STL)

Under `builds/tests/13kn_sl_fast/`, verify every item exists and is non-empty:

- Nozzle/chamber/jacket: `wall.stl`, `jacket.stl`, `regen.stl`, `contour.csv`, `profile.png`, `regen_3d.png`, `summary.json`
- Injector: `pintle.json`, `pintle/pintle_parameters.json`, `pintle/pintle_dimensions.csv`, and (with CadQuery) per-body pintle STLs — expect ≥ 9 bodies: injector_faceplate, hollow_pintle_body, pintle_tip, axial_annulus, radial_slot_network, fuel_manifold, oxidizer_manifold, igniter_interface, regen_coolant_outlet
- Pump: `pump.json`, `pump_bom.json`, `pump/pump_parts/` with 11 STLs (fuel_/oxidizer_ impeller, inducer, diffuser_volute, motor, inverter; shared_battery_pack) plus `pump/pump_reference_assembly.stl`

Any missing STL is a FAIL (missing pintle STLs with no CadQuery = SKIPPED, per Step 0).
...
Reference snapshot from the verified fast run (2026-07-02, seed contour): Isp 254.2 s, Cf 1.441, c* 1774 m/s (delivered 1730), mdot 5.215 kg/s (O/F 2.27), Rt 30.94 mm, Me 2.78, sized t_hot 0.60 mm with 194 × 0.50 mm channels, peak wall 946 K, coolant Δp ~5.6 bar, thermal margin 1.06, σ-margin ≥ 1.7. Values in this ballpark on a fresh clone are healthy.
