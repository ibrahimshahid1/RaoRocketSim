...
class MissionSpec:
    ...
    OF: float = 2.3                 # oxidizer/fuel mass ratio
    gamma: float = 1.24
    R_gas: float = 346.0            # J/(kg K)
    Tc: float = 3550.0              # K
    eta_cstar: float = 0.94         # delivered = eta * ideal (plan rule 2)
    ...
    throat_rd_factor: float = 1.5   # throat downstream curvature / Rt
    ...
    # --- design margins (§10.3; ALL default 1.0 = nominal conditions) ---------
    ...
    #
    #   Defaults are 1.0 so nominal results are unchanged and reproducible; set
    #   them (or use --design-margins on the CLI) to re-run any optimum as a
    #   *design point* rather than a nominal-conditions answer.
    ...
    film_capacity_margin: float = 1.0    # SP-8087 film system capacity: 2.0
    ...
    liner_sigma_allow: float = 200.0e6  # Pa, allowable at temperature
    ...
    #   **40–50 in for LOX/RP-1**; 43 in ≈ 1.09 m is used.  Since V_c ≈ A_c·L,
    #   the barrel length follows as L ≈ L*·A_t/A_c = L*/CR — which is why the
    #   0.13 m default below is consistent (1.09/8 = 0.136 m) and why chamber
    #   length is, to first order, thrust-INDEPENDENT at fixed L* and CR.
    l_star: float = 1.09                  # m (43 in), LOX/RP-1 (SP-125 Tab 4-1)
    chamber_length: float = 0.13          # m barrel length (= L*/CR)
    ...
    cooling_fraction: float = 1.0         # fraction of fuel through jacket
    ...
    def for_thrust(cls, thrust: float, *, Pc_ref: float = 3.0e6,
        ...
        # 2. barrel length from L*
        chamber_length = probe.l_star / CR
    ...
    def scaled_design_space(self) -> tuple["VariableSpec", ...]:
        ...
        return (
            ...
            VariableSpec("film_frac", 0.0, 0.30, 0.0),
...
class DesignVector:
    ...
    def from_array(cls, x: Array) -> "DesignVector":
        """Build from a 4- OR 6-long array.  A 4-long vector keeps D_pintle /
        N_rpm at their defaults (the skeleton path); a 6-long vector sets all
        variables (the whole-engine / NLP path)."""
        x = jnp.asarray(x)
        n = int(x.shape[0])
